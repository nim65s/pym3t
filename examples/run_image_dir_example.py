...
def parse_script_input():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to directory where "rbg*" and "depth*" named images are stored',
    )
    parser.add_argument("-n", "--nb_img_load", dest="nb_img_load", type=int, default=-1)
    parser.add_argument(
        "-s" "--stop_at_each_img",
        dest="stop_at_each_img",
        action="store_true",
        default=False,
    )
    parser.add_argument(
...
img_bgr_lst = [
    cv2.imread(name, color_read_flags) for name in color_names
]  # loads a dtype=uint8 array
depth_read_flags = cv2.IMREAD_GRAYSCALE + cv2.IMREAD_ANYDEPTH
img_depth_lst = [
    cv2.imread(name, depth_read_flags) for name in depth_names
]  # loads a dtype=uint8 array
